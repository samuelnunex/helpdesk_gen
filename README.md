...
- **Auth & DB**: PostgreSQL (Drizzle ORM), JWT (jose), bcrypt  
- **Tooling & DX**: Biome, Husky  

### Autenticação (PostgreSQL + JWT)

O template inclui autenticação com JWT e persistência em PostgreSQL:

1. **Variáveis de ambiente**  
   Copie `.env.example` para `.env` e defina:
   - `DATABASE_URL` – connection string do Postgres (ex.: local, Supabase, Neon)
   - `JWT_SECRET` – chave secreta para assinar os tokens (em produção use algo como `openssl rand -base64 32`)
   - `JWT_EXPIRES_IN` – (opcional) ex.: `7d`

2. **Migração do banco**  
   A tabela de usuários é `usuarios`, com colunas em português: `id`, `email`, `senha_hash`, `nome`, `criado_em`, `atualizado_em`. Crie a tabela com:
   ```bash
   npm run db:migrate
   ```
   Ou execute manualmente o SQL em `drizzle/0000_init_users.sql`. Se você já tinha a tabela `users` (em inglês), faça backup dos dados e recrie com o novo schema ou adapte a migração.

3. **Fluxo**  
   - Registro: `/auth/register` → `POST /api/auth/register`  
   - Login: `/auth/login` → `POST /api/auth/login` (define cookie httpOnly com JWT)  
   - Logout: `POST /api/auth/logout`  
   - Usuário atual: `GET /api/auth/me`  
   - O middleware redireciona rotas protegidas para o login quando não houver cookie de autenticação.

## Telas

### Disponíveis
- Painel inicial (Default)  
- Painel CRM  
- Painel Finanças  
- Login e Cadastro (`/auth/login`, `/auth/register`)  
